...
public class CommandSuggestor {
    ...
    public CommandSuggestor(Minecraft client, EditBox field) {
        this.client = client;
        this.field = field;
    }
    ...
    public void refresh() {
        ...
        try {
            ...
            ParseResults<ClientSuggestionProvider> parse = dispatcher.parse(reader, connection.getSuggestionsProvider());
            pendingInput = text;
            pending = dispatcher.getCompletionSuggestions(parse, field.getCursorPosition());
        } catch (Exception e) {
            pending = null;
        }
    }
    ...
    private void pollPending() {
        ...
        if (result == null || result.isEmpty() || !field.getValue().equals(pendingInput)) {
            entries = Collections.emptyList();
            visible = false;
            field.setSuggestion(null);
            return;
    ...
    private boolean applySelected() {
        ...
        int cursor = Math.min(suggestion.getRange().getStart() + suggestion.getText().length(), field.getValue().length());
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double amount) {
        ...
        firstVisible = Mth.clamp(firstVisible - (int) Math.signum(amount), 0, max);
        return true;
    }
    ...
    public void render(GuiGraphicsExtractor context, int mouseX, int mouseY, int screenWidth, int screenHeight) {
        ...
        for (int i = firstVisible; i < firstVisible + count; i++) {
            textWidth = Math.max(textWidth, client.font.width(entries.get(i).getText()));
        }

        windowW = textWidth + 6;
        windowH = count * ENTRY_HEIGHT;

        int anchorX = field.getScreenX(Math.max(0, entryRangeStart - 1));
        windowX = Mth.clamp(anchorX - 3, 0, Math.max(0, screenWidth - windowW));
        ...
        for (int i = 0; i < count; i++) {
            ...
            context.text(client.font, entries.get(index).getText(), windowX + 3, entryY + 2,
                    index == selectedIndex ? SELECTED_COLOR : TEXT_COLOR, false);
        }

        if (entries.size() > count) {
            String counter = (selectedIndex + 1) + "/" + entries.size();
            context.text(client.font, counter,
                    windowX + windowW - client.font.width(counter) - 3,
